...
FAQ 하단에는 사용자 질문 제목이 최신순으로 10건씩 표시됩니다. 질문 내용과 답변은 작성자가 게시글 비밀번호를 확인한 경우에만 표시됩니다. Mock 비회원은 작성 후 7일, Mock 회원은 영구보관 예정 안내가 표시되며, 작성자는 잠금 해제 후 수정·삭제 또는 답변 후 **수정해서 다시 질문**을 사용할 수 있습니다. 관리자 Mock 역할에는 공지 FAQ 관리 메뉴가 나타납니다.
...
상세 수동 점검 순서는 [Frontend Mock 테스트 체크리스트](./docs/프론트엔드%20Mock%20테스트%20체크리스트.md)를 확인하세요.
