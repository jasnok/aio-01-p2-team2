...
def test_search_consultations_calls_consultation_tool(monkeypatch) -> None:
    received = {}
    ...
    result = asyncio.run(
        legal_mcp.search_consultations(
            query="할부 결제한 물건이 배송되지 않았습니다.",
            category="consumer",
        )
    )

    assert received["server_name"] == "legal"
    assert received["tool_name"] == "search_consultations"
