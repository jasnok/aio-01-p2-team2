...
def _render_question_detail(item: dict) -> None:
    ...
    if item.get("is_owner") and item["status"] == "PENDING":
        with st.expander("내 질문 수정·삭제"):
            ...
            with st.form(f"api-question-delete-{item['id']}"):
                ...
                if st.form_submit_button("질문 삭제"):
                    if _call(backend_client.delete_question_api, token, guest_id, item["id"], password) is not None:
                        st.rerun()
...
def render_api_community_faq(category_code: str) -> None:
    ...
    with st.expander("질문 작성"):
        with st.form("api-question-create", clear_on_submit=True):
            category = st.selectbox("분야", list(CATEGORY_LABELS), index=list(CATEGORY_LABELS).index(category_code))
            title = st.text_input("제목", max_chars=100)
            content = st.text_area("질문 내용", max_chars=2000)
            password = st.text_input("게시글 비밀번호", type="password", max_chars=20)
            private = st.checkbox("🔒 비밀글로 작성", value=True)
            privacy = st.checkbox("개인정보를 작성하지 않았습니다.")
            if st.form_submit_button("질문 등록", type="primary", use_container_width=True):
                ...
                if _call(backend_client.create_question_api, token, guest_id, body) is not None:
    ...
    for item in payload["items"]:
        with st.container(border=True):
            st.caption(f"{CATEGORY_LABELS[item['category']]} · {'🌐 공개글' if item['visibility'] == 'PUBLIC' else '🔒 비밀글'} · {STATUS_LABELS[item['status']]} · {item['display_name']} · {_format_time(item.get('created_at'))}")
